...
# ============================================================
# NextCampus — IIT Pages Generator
# Generates all 21 remaining IIT college pages
# ============================================================

$basePath = "C:\Users\Satya\OneDrive\Desktop\NextCampus\frontend\colleges"
$lpuCss = "$basePath\lpu\lpu.css"
...
$iits = @(
    ...
    @{ slug = "iitpkd"; name = "IIT Palakkad"; abbr = "IITPKD"; city = "Palakkad"; state = "Kerala"; est = 2015; nirf = 55; qs = 0; campus = 600; students = 1500; programs = 15; rate = 76; high = "&#8377;35 LPA"; avg = "&#8377;8-11 LPA"; companies = 80; website = "https://iitpkd.ac.in" },
    @{ slug = "iittirupati"; name = "IIT Tirupati"; abbr = "IITTP"; city = "Tirupati"; state = "Andhra Pradesh"; est = 2015; nirf = 55; qs = 0; campus = 543; students = 1500; programs = 15; rate = 76; high = "&#8377;35 LPA"; avg = "&#8377;8-11 LPA"; companies = 80; website = "https://iittp.ac.in" },
...
function Get-HtmlPage($iit) {
    $nirfBadge = if ($iit.nirf -gt 0) { "&#127942; NIRF #$($iit.nirf)" } else { "&#127942; New IIT" }
    $nirfRow = if ($iit.nirf -gt 0) { "<tr><td>NIRF (Engineering)</td><td><strong>#$($iit.nirf)</strong></td><td>2024</td></tr>" } else { "<tr><td>NIRF</td><td>Emerging</td><td>2024</td></tr>" }
    $qsRow = if ($iit.qs -gt 0) { "<tr><td>QS World Rankings</td><td>#$($iit.qs)</td><td>2025</td></tr>" }         else { "" }
            ...
            <div class="lpu-hero-grid">
                <div class="lpu-hero-info">
                    ...
                    <div>
                        ...
                        <div class="lpu-ctas">
                            <a href="$($iit.website)/" target="_blank" class="btn-lpu-apply">Apply Now &#8594;</a>
                            <a href="#" class="btn-lpu-brochure" onclick="alert('Brochure coming soon!');return false;">&#128196; Download Brochure</a>
                        </div>
                    </div>
                </div>
            </div>
...
function Get-JsPage($iit) {
    return @"
/* $($iit.name) ($($iit.abbr)) — NextCampus College Detail JS */
...
foreach ($iit in $iits) {
    $dir = "$basePath\$($iit.slug)"
    New-Item -ItemType Directory -Force -Path "$dir\images\logo" | Out-Null
    New-Item -ItemType Directory -Force -Path "$dir\images\cover" | Out-Null

    # CSS — exact copy of lpu.css
    Copy-Item $lpuCss "$dir\$($iit.slug).css" -Force

    # HTML
    $html = Get-HtmlPage $iit
    [System.IO.File]::WriteAllText("$dir\$($iit.slug).html", $html, [System.Text.Encoding]::UTF8)

    # JS
    $js = Get-JsPage $iit
    [System.IO.File]::WriteAllText("$dir\$($iit.slug).js", $js, [System.Text.Encoding]::UTF8)

    Write-Host "✅ Created: $($iit.name) ($($iit.slug))"

    # home.js entry
    $homeJsEntries += "    {`n      name: '$($iit.name) ($($iit.abbr))',`n      city: '$($iit.city)', state: '$($iit.state)', type: 'Government',`n      score: 9.$([math]::Max(1, 10 - $iit.nirf)).toFixed(1), totalFees: '&#8377;8L — 10L', avgPackage: '$($iit.avg)',`n      placementRate: $($iit.rate), nirf: $($iit.nirf),`n      link: '../colleges/$($iit.slug)/$($iit.slug).html',`n      rating: '4.5', accr: 'NIRF #$($iit.nirf)'`n    },"
}

Write-Host "`n✅ All $($iits.Count) IIT pages generated successfully!"
Write-Host "`n--- home.js entries (paste after existing IIT entries) ---"
$homeJsEntries | ForEach-Object { Write-Host $_ }
