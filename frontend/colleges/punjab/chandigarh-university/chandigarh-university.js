...
/* Chandigarh University (CU) College Detail Page — JavaScript */
/* Exact same logic as lpu.js — framework is reused as-is */
...
function setupTabs() {
    ...
    function switchTab(tabName) {
        ...
        const matchingTab = [...tabs].find(t => t.dataset.tab === tabName);
    ...
    // "View All" buttons in the overview panel
    document.querySelectorAll('.btn-view-tab').forEach(btn => {
        btn.addEventListener('click', () => switchTab(btn.dataset.target));
    });
}

/* ===== Sticky Header (shows on scroll past hero) ===== */
function setupStickyHeader() {
    const stickyHeader = document.getElementById('lpu-sticky-header');
    const hero = document.getElementById('lpu-hero');
    const tabsNav = document.getElementById('lpu-tabs-nav');
    ...
    window.addEventListener('scroll', () => {
        ...
        if (scrollY > heroBottom) {
            ...
            if (tabsNav) {
                tabsNav.style.top = stickyHeader.offsetHeight + 'px';
            }
        } else {
            stickyHeader.classList.remove('visible');
            if (tabsNav) {
                tabsNav.style.top = '0px';
            }
        }
